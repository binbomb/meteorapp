...
Template.Calendar_Page.onRendered(() => {
  ...
  $('#event-calendar').fullCalendar({
	...
	dayClick(date, session) {
	  if (session.originalEvent.type === "scroll") {
		return; 
	  }
		console.log(session.originalEvent.type )//session is event
      // Store the date so it can be used when adding an event to the EventData collection.
	   console.log(Session.get('eventModal', { type: 'add', date: date.format() }))
      Session.set('eventModal', { type: 'add', date: date.format() });
      // If the date has not already passed, show the create event modal.
	  console.log(date.format())
	  console.log(moment())
	  console.log((moment(date.format()).isSameOrAfter(moment(), 'day')) )
      if(moment(date.format()).isSameOrAfter(moment(), 'day')) {
       // $('#create-event-modal').modal({ blurring: true }).modal('show');
	  // // console.log(('#create-event-modal'))
	    $('#create-event-modal').css('display','block');
      }
	  //  $('#create-event-modal').css('display','block');
    },
