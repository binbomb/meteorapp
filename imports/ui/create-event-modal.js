...
import { EventData, EventDataSchema } from '../../collections/eventdata.js';
...
Template.Create_Event_Modal.onRendered(function enableSemantic() {
	// console.log(this.$('.ui.fluid.search.dropdown')[1])
...
Template.Create_Event_Modal.events({
  'submit .session-data-form'(event, instance) {
    ...
    let start = `${newEvent.date}T${f.options[f.selectedIndex].value}+07:00`;
    ...
    let end = `${newEvent.date}T${g.options[g.selectedIndex].value}+07:00`;
...
//EventData.insert(newEvent);
    if (instance.context.isValid()) {
      EventData.insert(newEvent);
	 // // console.log("thah conpg")
      instance.messageFlags.set(displayErrorMessages, false);
      $('#create-event-modal').css('display','none')
        //  .modal('hide')
      ;
      FlowRouter.go('two');
    } else {
